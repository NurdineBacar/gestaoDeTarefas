...
const login = async () => {
    ...
    try {
        const response = await fetch('http://cursophp.com/gestaoDeTarefas-master/src/backend/controllers/login.php', {
            method: 'POST',
            headers: {
        ...
        try {
            ...
            if (result.success) {
                window.location.href = "http://localhost:5173/panel";
            } else {
                alert(result.message);
            }
        } catch (jsonError) {
            console.error("Resposta não é JSON:", text);
            alert("Erro ao fazer login. Resposta do servidor não é JSON.");
        }
